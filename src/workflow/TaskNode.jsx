...
import { Handle, Position } from '@xyflow/react';
 ...
 const TaskNode = ({ data, isConnectable, id, type }) => {
    ...
    <>
      ...
      <div>
        <ul className="w-48 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
          <li className="w-full px-4 py-2 border-b border-gray-200 rounded-t-lg dark:border-gray-600">{data?.title}</li>
          <li className="w-full px-4 py-2 border-b border-gray-200 dark:border-gray-600">Due date: {data?.dueDate}</li>
          <li className="w-full px-4 py-2 border-b border-gray-200 dark:border-gray-600">Status: {data.status}</li>
          <li className="w-full px-4 py-2 rounded-b-lg">Description: {data.description}</li>
      </ul>
      </div>
      <Handle
        type="source"
